...
RSpec.describe Github::GithubContributorsImporter do
  describe '.call' do
    ...
    def stub_contributors(*logins)
      allow_any_instance_of(Octokit::Client).to receive(:contributors)
        .with(repository.github_full_name)
        .and_return(logins.map { |login| github_contributor(login) })
    end
